...
class XieChengSpider(object):
    ...
    def __init__(self, pageIndex=1, url="http://www.mafengwo.cn"):
    ...
    def fenlei(self,content_list):
        ...
        for idx, item in enumerate(types):
            with open('/Users/pujingdadi/Desktop/pachong2/' + str(names[idx]) + '.txt', 'a+', newline='') as f:
                for row in item:
                    f.write(row)
                    f.write('\n')
    ...
    def saveFile(self, data):
        filename = "/Users/pujingdadi/Desktop/pachong2/" + "xiecheng.txt"
        if not os.path.exists(filename):
            os.system(r"touch {}".format(filename))  # 调用系统命令行来创建文件
        path = "/Users/pujingdadi/Desktop/pachong2/" + "xiecheng.txt"
    ...
    def getBlogInfo(self, page_index):
        ...
        url = self.url + "/search/s.php?q=%s" % word + "&p=" + str(page_index) + "&t=wenda&kt=1"
        # print(url)
        ...
        for link in soup.find_all('h3'):
            ...
            if link.get_text() != '旅游攻略导航':
                ...
                res.append(link.find('a').get_text())
        # print(res)
        return res
